import {applyCartCoupon} from '../studio/src/cart-campaigns.js';
...
const ratingInput=(id,fallback)=>{const input=$('#'+id);return input?.value&&input.validity.valid?input.value.replace('.',','):fallback;};
...
document.querySelectorAll('[data-client-person]').forEach(button=>button.onclick=()=>{selectedPerson=Number(button.dataset.clientPerson);const r=campaign.recipients[selectedPerson];$('#client-request [name=audience]').selectedIndex=selectedPerson;$('#client-name').value=r.first_name;previousName=r.first_name;$('#client-company').value=client.kind==='cart-recovery'?r.last_name:r.company;previousLastName=r.last_name;$('#client-note').value=r.personal_note;$('#client-salutation').value=r.salutation;if(client.kind==='cart-recovery'){$('#cart-product-name').value=r.product_name;$('#cart-product-variant').value=r.product_variant;$('#cart-return-url').value=r.cart_url;}if($('#rating-current')){$('#rating-current').value=r.rating_current.replace(',','.');$('#rating-example').value=r.rating_example.replace(',','.');updateRatings();}document.querySelectorAll('[data-client-person]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));render();});
let previousName=$('#client-name').value,previousLastName=campaign.recipients[0].last_name;
$('#client-company').addEventListener('input',()=>{if(client.kind!=='cart-recovery')return;const value=$('#client-company').value;if(previousLastName&&$('#client-salutation').value.includes(previousLastName))$('#client-salutation').value=$('#client-salutation').value.replace(previousLastName,value);previousLastName=value;});
