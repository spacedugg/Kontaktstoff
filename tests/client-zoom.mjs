...
try{
 ...
 await page.locator('#client-studio').click();const draft=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('kontaktstoff-client-draft')));assert.ok(draft.name.includes('Angebotsidee'));assert.ok(draft.sides.back.fields.some(f=>f.text.includes('Erste erfolgreiche Löschung gratis.')));
