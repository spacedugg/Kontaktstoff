...
await build({stdin:{contents:"export {createBewertungspush,applyBewertungspushOffer} from './studio/src/bewertungspush.js';export {renderCanvas,drawRatingStars} from './studio/src/render.js';",resolveDir:process.cwd()},bundle:true,format:'esm',outfile:'tmp/pdfs/offer-check.js'});
...
try{
 ...
 await page.locator('.rating-settings summary').click();
 const pixelsBefore=await page.locator('#flat-front').evaluate(c=>c.toDataURL());
 await page.locator('#rating-current').fill('3.8');await page.locator('#rating-example').fill('4.6');await expect(page.locator('#client-stage')).toHaveAttribute('aria-busy','false');await expect(page.locator('.rating-settings summary span')).toHaveText('3,8 → 4,6');assert.notEqual(await page.locator('#flat-front').evaluate(c=>c.toDataURL()),pixelsBefore);
 ...
 await page.locator('#client-studio').click();const draft=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('kontaktstoff-client-draft')));assert.ok(draft.name.includes('Angebotsidee'));assert.equal(draft.recipients[0].rating_current,'3,8');assert.equal(draft.recipients[0].rating_example,'4,6');assert.equal(draft.sides.front.fields.filter(f=>f.display==='stars').length,2);assert.ok(draft.sides.back.fields.some(f=>f.text.includes('Erste erfolgreiche Löschung gratis.')));
 ...
 const gold=await page.evaluate(async()=>{const {drawRatingStars}=await import('/tmp/pdfs/offer-check.js');return ['4,2','4,7','0','5','invalid'].map(value=>{const canvas=document.createElement('canvas');canvas.width=550;canvas.height=100;const ctx=canvas.getContext('2d');drawRatingStars(ctx,{x:0,y:0,w:550,h:100,color:'#efad27'},value);const data=ctx.getImageData(0,0,550,100).data;let count=0;for(let i=0;i<data.length;i+=4)if(data[i]===239&&data[i+1]===173&&data[i+2]===39&&data[i+3]===255)count++;return count;});});assert.ok(gold[1]>gold[0]&&gold[3]>gold[1]);assert.equal(gold[2],0);assert.equal(gold[4],0);
 console.log('Dynamic rating values, fractional stars, Zoom, side selection, drag, fit, focus, mobile, optional offer layout and studio handoff passed.');
