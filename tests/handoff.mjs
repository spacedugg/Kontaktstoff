...
const recipients=Array.from({length:3},(_,i)=>({id:uid(),company:'Unternehmen '+(i+1),first_name:'Anna',salutation:'Hallo Anna,',personal_note:'Eine persönliche Idee für euer Team: Lasst uns über eure nächste Kampagne sprechen.',chatbot_url:'https://example.org/demo/'+i,street:'Hafenstraße 12',postal_code:'01067',city:'Dresden',country:'Deutschland'}));
...
try{
 ...
 for(const t of TEMPLATES){const c=createTemplate(t.id);c.recipients=structuredClone(recipients);c.onboarding.active=false;await upload(c);await page.locator('#open-preview').click();await page.locator('#audit-all').click();await expect(page.locator('#audit-results')).toContainText('Alle Empfänger geprüft');await expect(page.locator('#audit-results')).not.toContainText('Fehler gefunden');await expect(page.locator('#audit-results')).toContainText('0 Hinweise');}
