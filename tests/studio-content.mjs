...
try{
 ...
 await page.locator('[data-workflow-next]').click();await page.locator('#contact-add-empty').click();await page.locator('[data-key=company]').fill('Mein Testteam');await page.locator('[data-key=first_name]').fill('Mara');await page.locator('#contact-columns').check();await page.locator('[data-key=salutation]').fill('Hallo Mara,');await page.locator('[data-key=personal_note]').fill('Nach eurem Projektabschluss laden wir euch auf einen Kaffee ein.');await page.locator('[data-key=chatbot_url]').fill('https://example.org/mein-testteam');
