...
try{
 ...
 await page.locator('[data-guide-action="next"]').click();await expect(page.locator('#setup-view')).toContainText('Deine Karte ist bereit');await shot('guide-step-6');await page.locator('[data-guide-action="finish"]').click();await expect(page.locator('#three-d-view')).toBeVisible();await page.waitForTimeout(500);
 ...
 await expect(page.locator('#guide-return')).toBeHidden();assert.equal((await project()).onboarding.active,false);
 ...
 // A completed guide no longer competes with editor navigation. A new guided draft remains available.
 await expect(page.locator('[data-tab="setup"]')).toBeHidden();assert.equal((await project()).name,'Hotelkampagne Herbst');
 await page.goto('http://127.0.0.1:4177/studio/?start=blank');await page.locator('#setup-name').fill('Responsive Entwurf');
 for(const width of [1024,768,390]){await page.setViewportSize({width,height:900});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),`Guide overflow at ${width}`);await shot(`guide-${width}`);}
 await page.locator('.guide-step-picker summary').click();await page.locator('[data-guide-step="5"]').click();await page.locator('[data-guide-action="finish"]').click();await page.waitForTimeout(500);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await shot('3d-mobile');
