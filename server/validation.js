import {validateCampaign,validURL} from '../studio/src/core.js';
...
export function payload(input){
 ...
 meta.formatRequest=data.formatRequest||'a5';if(!['a5','selfmailer','special'].includes(meta.formatRequest))throw new HTTPError(400,'Ungültiges Wunschformat.');
 ...
 if(!['upload','research'].includes(meta.leadSource)||!['appointment','demo','offer','event'].includes(meta.goal))throw new HTTPError(400,'Ungültige Auswahl.');
...
export function requestIssues(project,meta,company){
 ...
 if(!meta.designService&&Object.values(project.sides).some(s=>s.background.kind==='blank'&&!s.fields.length))issues.push('Beide Kartenseiten gestalten oder Gestaltung anfragen.');
