...
- **Kampagnen**: Ein Name reicht für den ersten Entwurf. Ohne Anmeldung bleibt er in IndexedDB in diesem Browser gespeichert; „Weiter“ und „Entwurf anlegen“ öffnen keine Anmeldung. „Entwurf online speichern“ fordert bewusst Anmeldung/Registrierung an und übernimmt genau diesen Entwurf ins Konto. Ein Konto gilt für alle Kampagnen. Zielgruppe und Design sind mit „Auch später möglich“ gekennzeichnet. Fehlt eine Auswahl oder enthält die Zielgruppe noch keine Kontakte, wird die Personalisierungsprüfung zunächst übersprungen. Design und CSV-Kontakte lassen sich über die Links im gespeicherten Entwurf im Editor ergänzen. Bei vorhandenen Quellen prüft der Server Besitz und Versionsnummern und speichert Kopien inklusive Quellen-IDs/-Revisionen. Ohne Zielgruppe werden keine Beispielkontakte aus dem Design übernommen. Es gibt noch keinen automatisierten Shop-Import oder Versandplaner. Der Versandtag ist ein Wunschdatum.
...
## Interner Zugang unter /admin

`/admin/` bietet dem Kontaktstoff-Team einen Passwort-Einstieg ohne E-Mail-Registrierung. `ADMIN_PASSWORD_HASH` muss ausschließlich serverseitig als `scrypt`-Hash (`32 Hexzeichen Salt:128 Hexzeichen Digest`, 64 Bytes, Node-Standardparameter) konfiguriert werden. Es gibt kein Standardpasswort im Quellcode. Ohne gültige Konfiguration bleibt der Zugang geschlossen.

Der reservierte Benutzer `kontaktstoff-internal-admin` hat einen eigenen Kampagnenarbeitsplatz und Zugriff auf den bestehenden Anfragen-Eingang. Kundenkonten bekommen diese Rolle nicht. Der normale E-Mail-Login und die Registrierung akzeptieren die interne Identität nicht. Die Anmeldung nutzt Same-Origin-Prüfung, persistente Fehlversuchslimits, HttpOnly/Secure-Cookies und die bestehenden CSRF-Prüfungen. Ein Wechsel oder Entfernen des Passwort-Hashes sperrt bestehende Admin-Sitzungen; erneute Anmeldung mit geändertem Hash entfernt die alten Sitzungen. Kunden-Freigabelinks funktionieren weiterhin ohne Konto.

### Vereinfachte Designabstimmung

Die Kundenansicht öffnet beim Klick auf Vorder- oder Rückseite ein Kommentarfeld. Der Pin wird mit Seite und normalisierten Koordinaten gespeichert; auf dem Handy öffnet sich dasselbe Dialogfenster. Allgemeine Kommentare bleiben möglich. Fehler erscheinen direkt im Kommentardialog. Im Backend heißt die Bearbeitung „Design bearbeiten“. „Änderungen für Kunden übernehmen“ erscheint nur bei einem abweichenden Quelldesign und veröffentlicht weiterhin einen neuen unveränderlichen Stand.

Freigaben können über `POST /api/reviews/:id/delete` in den Papierkorb verschoben werden. Der Link wird deaktiviert, während Kampagne, Design, Kommentare und Versionsstände erhalten bleiben. `GET /api/reviews/trash` zeigt nur eigene gelöschte Freigaben; `POST /api/reviews/:id/restore` stellt den vorherigen Status wieder her. Beide Änderungen sind versionsgeschützt. Die additive Tabelle `review_trash` hält den vorherigen Status fest. Ein abgelaufener Link bleibt nach Wiederherstellung abgelaufen.
